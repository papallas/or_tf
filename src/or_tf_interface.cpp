    ...
    switch(type) {
        case OpenRAVE::PT_Sensor:
            if( interfacename == "or_tf") {
                ...
                if (sinput.fail()) {
                    RAVELOG_ERROR("or_tf is missing the node_name, and/or openrave_tf_frame parameter(s).\n");
                ...
                OpenRAVE::SensorBasePtr tfp = OpenRAVE::SensorBasePtr(new OrTf(env,openrave_tf_frame));
...
void GetPluginAttributesValidated(OpenRAVE::PLUGININFO& info) {
    info.interfacenames[OpenRAVE::PT_Sensor].push_back("or_tf");
}
